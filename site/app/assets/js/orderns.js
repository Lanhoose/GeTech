...
async function salvarOrdem(){
  ...
  const r=push(ref(db,'ordens')); await set(r,ordem); ordens.push({id:r.key,...ordem}); renderizarTabela(); limparCampos();
}
...
async function excluirOrdem(){
 ...
 await remove(ref(db,`ordens/${ordens[idx].id}`)); ordens.splice(idx,1); renderizarTabela();
}
...
function logout(){auth.signOut().then(()=>location.href='../pages/login.html');}
