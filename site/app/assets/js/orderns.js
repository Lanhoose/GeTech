...
function trocarTela(tela,botao){
  ['tela-cadastro','tela-rastreio','tela-frete'].forEach(id=>{const el=document.getElementById(id);if(el)el.style.display='none';});
...
function calcularFrete(){const peso=Number(document.getElementById('peso')?.value),dist=Number(document.getElementById('distancia')?.value),tipo=Number(document.getElementById('tipo-entrega')?.value);if(!peso||!dist){alert('Preencha os dados.');return;}const resultado=document.getElementById('resultado-frete');if(!resultado)return;resultado.innerHTML=`R$ ${(((peso*.45)+(dist*.12))*tipo).toFixed(2)}`;}
