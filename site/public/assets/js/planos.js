...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Leva o usuário para o checkout da assinatura com o plano já selecionado.
    // A página de checkout (assinatura.html) exige login, seguindo o mesmo
    // padrão de proteção usado em toda a pasta public (protecao-gestor.js).
    btnConfirmar?.addEventListener('click', () => {
        const plano = document.getElementById('modalPlanName')?.innerText || 'Essencial';
        window.location.href = `assinatura.html?plano=${encodeURIComponent(plano)}`;
    });
